...
\begin{definition}[Pushforward probability measure]  Let $\mathcal{Z} \subseteq \mathbb{R}^{d}$ be a space, and $\mathcal{X} \subseteq \mathbb{R}^{d}$ a (data) space. A probability measure $\nu$ on $\mathcal{Z}$ and a (measurable) function:
  \[
...
\begin{remark}[Implicit (likelihood-free) Generative models] We wish to find $\theta$  that will optimize~\cref{eq:marginalLikelihood2}. 
